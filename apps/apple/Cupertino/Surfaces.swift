...
struct Surface: Identifiable, Hashable {
  ...
  // Generated bytes, where swift-format must have no opinion. `make surfaces`
  // writes this array from surfaces.json and `make surfaces-check` asserts it
  // byte for byte; a formatter that rewraps it makes the two rewrite each other
  // forever and fails the drift gate on a file nobody edited. The comment sits
  // outside the marker so regenerating cannot remove it.
  // swift-format-ignore
  ...
  static let all: [Surface] = [
    ...
    Surface(
      ...
      // tool. Here volume, mute, speak and the default device are mutations
      // that are NOT recording, and they have to be reachable without ever
      // enabling the microphone. Two independent gates, following the
      // allowCodes precedent: a change of tier gets its own switch rather
      // than allowWrites.
      //
