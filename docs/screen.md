...
**Verdict so far: GO on the capability, UNMEASURED on the identity.** ScreenCaptureKit does
everything the design needs, at 30 ms a window. What has not been measured is whether the grant
lands on `Cupertino.app` and stays there — the three questions in [Still open](#still-open) — and
those are the ones that decide whether this ships, because
[distribution.md](distribution.md) has the whole product resting on the grant landing correctly.
...
1. **Does an `LSUIElement` Developer-ID app get and keep the grant?**
2. **Does the grant survive `make install` moving the bundle?** Full Disk Access does, measured in
   [`scripts/spike-app-tcc`](../scripts/spike-app-tcc/README.md).
3. **Is there periodic re-consent on macOS 26?**
