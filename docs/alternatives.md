...
There is no first-party path **for Mail on macOS**. Anthropic ships a Gmail connector, not an
Apple Mail one, so nothing in Claude reads Mail.app on the desktop out of the box. Everything below
...
**And on macOS itself the ground shifted once more.** Anthropic now ships an official iMessage
plugin for Claude Code, in the same marketplace this project targets — the first first-party entry
on this page that runs on the platform this ships to. It is a _channel_ rather than a reader: its
job is to let a phone drive a Claude Code session, not to let Claude reason over a message archive,
so it does not close the hole this fills. What it does close is the installation one, in a single
command. See the row below.
...
| Project                                                                                                                                                       | Lane                                                            | Notes                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [anthropics/claude-plugins-official → `external_plugins/imessage`](https://github.com/anthropics/claude-plugins-official/tree/main/external_plugins/imessage) | chat.db direct + AppleScript send, as a Claude Code **channel** | **First-party, and not the same job.** Two tools (`reply`, `chat_messages`) and a poll loop: it exists so a phone can drive a Claude Code session, not so Claude can reason over an archive. Reads are scoped to an allowlist; there is no search, no counting, no attachment export and no contacts join. Installs in one command — the friction argument below, conceded. Still asks for Full Disk Access on **Terminal**. Two defects, measured rather than read: its `attributedBody` decoder reads one byte after the `0x81` length escape where the format puts an int16, so **every message of 128 bytes or more comes back corrupted** — a 400-byte message returns 144 bytes with a leading `U+0001`, a 256-byte one returns the empty string; and `chat_messages` has no `associated_message_type` predicate, so tapbacks render as messages the sender never typed. |
...
**No channel lane.** Recorded as a gap, not as a plan. Anthropic's iMessage plugin declares
`claude/channel` and `claude/channel/permission` and pushes `notifications/claude/channel`, which
is the host protocol that lets a phone drive a Claude Code session — including relaying tool
permission prompts to self-chat and reading `yes <id>` back. Nothing here references it, and
[surfaces.md](surfaces.md) frames a surface as a set of read/write tools, which a transport is not.

The reason to write it down rather than shrug: the one differentiator on this page applies to it
directly. A channel needs Full Disk Access to poll `chat.db`, and every implementation of one will
tell the user to grant that to Terminal. This project already puts a signed app at the top of that
launch chain. Whether that is worth a background poll loop, an access-control gate and a permission
relay — none of which exist here — is undecided and deliberately left so.
...
**Anthropic shipping one first-party sharpens that considerably.** `/plugin install imessage@…`
plus one relaunch is now the baseline a Messages user compares against, and it is set by the vendor
of the host. The grant still forces a download and a System Settings trip here, so the gap cannot
be closed the same way — but the comparison is no longer against a third-party plugin that happens
to be convenient. It is against the default. Whatever this page concludes about capability, that is
the row a prospective user reads first.
