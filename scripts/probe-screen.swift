...
func coveredFraction(of window: SCWindow, side: Int = 64) -> Double? {
  ...
  for i in 0..<side {
    for j in 0..<side {
      let pt = CGPoint(
        x: target.minX + (Double(i) + 0.5) / Double(side) * target.width,
...
func runProbe() async -> Int32 {
  ...
  if let err = enumerationError {
    ...
    row(
      "SCShareableContent", "DENIED — \(err.domain) \(err.code)\(declined ? " (userDeclined)" : "")"
    )
    print(
      """
  ...
  if !claimed {
    caveats.append(
      "flag said denied while enumeration succeeded — the two disagree, investigate before trusting either"
    )
  }
  ...
  if !titled.isEmpty {
    caveats.append(
      "enumeration exposes \(titled.count) window titles — a mail subject, a chat name, a document. list_targets must not return them by default."
    )
  }
  if unsafeTitles {
  ...
  for target in capturable {
    ...
    do {
      ...
      if !rendered {
        caveats.append(
          "\(target.surface) captured as a near-solid image (\(colours) colours) — likely blank")
      }
    } catch {
      ...
      caveats.append(
        "\(target.surface) capture threw \(err.code)\(named) — enumerable is not capturable, so list_targets must not promise a window it cannot grab"
      )
    }
  }
  ...
  if !anyRendered {
    blockers.append(
      "no window produced a non-blank capture — SCScreenshotManager returns pixels but not content")
    verdict = NO_GO
  }
  ...
  if let off = offScreenRendered {
    ...
    if !off {
      caveats.append(
        "an off-screen window captured blank — check the RESULT, never predict it from isOnScreen: a drawn window composites off screen too"
      )
    }
  }
  ...
  if buried.isEmpty {
    ...
    for b in buried {
      row(
        "  " + b.surface, String(format: "covered %.0f%%, colours %d", b.covered * 100, b.colours))
    }
    print(
