 ...
 * without resources would name a `cupertino://calendar/guide` that nothing serves.
 */
describe("calendar exposePrompts", () => {
  ...
  it("registers neither when it is off, and stops advertising the capability", async () => {
    const client = await connect({ APPLE_CALENDAR_EXPOSE_PROMPTS: "0" });
  ...
  it("leaves the tools untouched", async () => {
    ...
    const off = (await (await connect({ APPLE_CALENDAR_EXPOSE_PROMPTS: "0" })).listTools()).tools
      .length;
    expect(off).toBe(on);
