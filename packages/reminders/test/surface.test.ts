...
/**
 * Prompts and resources for the Reminders surface.
 *
...
describe("reminders resources", () => {
  it("registers the surface resources", async () => {
    expect(await uris(await connect())).toEqual([
      "cupertino://reminders/guide",
      "cupertino://reminders/diagnostics",
      "cupertino://reminders/inventory",
    ]);
  });
  ...
  it("serves the guide as markdown without touching a permission", async () => {
    const res = await (await connect()).readResource({ uri: "cupertino://reminders/guide" });
  ...
  it("answers a diagnostics read with data even when nothing is reachable", async () => {
    const res = await (await connect()).readResource({ uri: "cupertino://reminders/diagnostics" });
    expect(res.contents[0]?.mimeType).toBe("application/json");
    expect(() => JSON.parse(textOf(res.contents))).not.toThrow();
  });
});

describe("reminders prompts", () => {
  it("registers the read-only prompts", async () => {
    expect(await promptNames(await connect())).toEqual(["apple_reminders_whats_due"]);
  });

  /*
   * The same invariant the mutating tools hold: with writes off a write prompt
   * must not merely refuse, it must be INVISIBLE. A visible "apple_reminders_capture_action_items" on a
   * read-only server is an offer the server cannot keep.
   */
  it("registers the write prompts only when writes are on", async () => {
    expect(await promptNames(await connect())).not.toContain(
      "apple_reminders_capture_action_items",
    );
    expect(await promptNames(await connect({ APPLE_REMINDERS_ALLOW_WRITES: "1" }))).toEqual([
      "apple_reminders_capture_action_items",
      "apple_reminders_whats_due",
    ]);
  });
  ...
  it("embeds the guide ahead of the instruction", async () => {
    const got = await (
      await connect()
    ).getPrompt({ name: "apple_reminders_whats_due", arguments: {} });
    expect(got.messages[0]?.content).toMatchObject({
      type: "resource",
      resource: { uri: "cupertino://reminders/guide", mimeType: "text/markdown" },
    });
 ...
 * without resources would name a `cupertino://reminders/guide` that nothing serves.
 */
describe("reminders exposePrompts", () => {
  ...
  it("registers neither when it is off, and stops advertising the capability", async () => {
    const client = await connect({ APPLE_REMINDERS_EXPOSE_PROMPTS: "0" });
  ...
  it("leaves the tools untouched", async () => {
    ...
    const off = (await (await connect({ APPLE_REMINDERS_EXPOSE_PROMPTS: "0" })).listTools()).tools
      .length;
    expect(off).toBe(on);
