/**
 * The only place in this package that spawns a process.
 *
 * ## Why this is safe
 *
 * `execFile`, never `exec` — there is no shell, so there is no quoting question
 * to get wrong. This mirrors the existing house precedent in mcp-x-api's
 * `compose/open.ts`.
 *
...
export const mapOsaError = (stderr: string, timeoutMs: number): Error => {
  const code = /\((-\d{3,4})\)\s*$/m.exec(stderr.trim())?.[1];
  switch (code) {
    case "-1743":
      return new TccDeniedError();
    case "-600":
    case "-609":
      return new MailNotRunningError();
    case "-1712":
      return new MailBusyError();
    default:
      break;
  }
...
/**
 * Serialise every invocation. Mail's Apple Event dispatch is single-threaded:
...
const defaultExec: ExecImpl = (path, args, script, timeoutMs) =>
  new Promise((resolve, reject) => {
    const child = execFile(
      ...
      (err, stdout, stderr) => {
        ...
        if (killed) {
          reject(new OsascriptTimeoutError(timeoutMs));
          return;
        }
        if ((err as NodeJS.ErrnoException).code === "ENOENT") {
          reject(
            new PlatformError(
              `${path} not found. This server only runs on macOS with Apple Mail installed.`,
            ),
          );
          return;
        }
        reject(mapOsaError(stderr || String(err.message), timeoutMs));
      },
    );
...
export const createOsascriptRunner = (opts: OsascriptOptions): OsascriptRunner => {
  ...
  const run = async <T>(script: string, params?: unknown): Promise<T> => {
    ...
    if (!envelope.ok) {
      ...
      if (code === "MAIL_NOT_RUNNING") throw new MailNotRunningError();
      if (code === "NOT_AUTHORIZED") throw new TccDeniedError();
...
/** Retry a Mail-busy failure once. Mail returns -1712 while mid-sync and succeeds moments later. */
export const withBusyRetry = async <T>(fn: () => Promise<T>, delayMs = 1500): Promise<T> => {
  try {
    return await fn();
  } catch (err) {
    if (!(err instanceof MailBusyError)) throw err;
